...
@export var goto_state: GameConfig.GAME_STATE = GameConfig.GAME_STATE.NONE
...
func _unhandled_input(_event: InputEvent) -> void:
	if Input.is_anything_pressed() and any_key_to_next and not did_go_to_next and visible:
		...
		if goto_state != GameConfig.GAME_STATE.NONE:
				GameManager.game_config.set_state(goto_state)
